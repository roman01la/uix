...
(defn compile-element [v {:keys [env] :as opts}]
  (let [tag (first v)]
    (if (uix.lib/cljs-env? env)
      (if (and (symbol? tag) (-> (ana/resolve-var env tag) :meta :uix/context))
        (compile-element* (into [`(.-Provider ~tag)] (rest v)) opts)
        (compile-element* v opts))
      (if (and (symbol? tag) (-> tag resolve meta :uix/context))
        (let [[ctx {:keys [value]} & children] v]
          [:uix/bind-context `(fn [f#] (binding [~ctx ~value] (f#))) (vec children)])
        v))))
